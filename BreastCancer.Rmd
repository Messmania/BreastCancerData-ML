...
plotDistributionNoClass<- function(db, target, xTitle){
  ...
  ggarrange(p1, p2, nrow=2, ncol=1, common.legend = T, legend="bottom")
}
```
...
#TODO see if 4 coupled plots can have 1 common legend, right now it is showing as 2 legends, one for each set of plot.
```
...
p1 <- plotDistributionNoClass(cleaned_data, cNames$singleEpiCellSize, "Single epithelial cell size with outliers")
p2 <- plotDistributionNoClass(temp, cNames$singleEpiCellSize, "Single epithelial cell size without outliers")
ggarrange(p1, p2, nrow=1, ncol=2)

cleaned_data <- temp
```

Seeing the distribution, it doesn't change much with the removal of outliers.

```{r Remove other outliers, fig.width=10}
hasOutliers(cleaned_data, cleaned_data$normalNucleoli) #NONE
hasOutliers(cleaned_data, cleaned_data$blandChromatin)  #NONE
```
...
### Final Random Forest
```{r Creating Random Forest again}
set.seed(137)
...
So we will calculate the distance of each datapoint in this PC plot to the centroid of the benign cluster. So the farther a point is from that centroid, more chances of it being malignant, hence we can call this distance the degree of malignancy of a tumor.
...
checkLinearity(cleaned_data, cNames$bareNuclei, cNames$degreeOfMalignancy) #OK
...
```{r Check Correlation}
cor(cleaned_data$uniformityCellSize, cleaned_data$clumpThickness_sq) #0.58
cor(cleaned_data$blandChromatin, cleaned_data$uniformityCellSize) #0.74
cor(cleaned_data$blandChromatin, cleaned_data$clumpThickness_sq) #0.51
```
...
RSE is only 0.78 and all individual coefficients are significant except interaction between clump thickness and its interaction with uniformity in cell size.
...
```
Our model overall shows a linear fit due to nearly equal up/down variations around the linear line.
...
checkHomoscedasticity(lm_centered$residuals, lm_centered$fitted.values)
```
Now all the vif scores are under 5. Also, note that our linear model has now improved with a tighter fit around the linear line after we centered the data.
...
checkHomoscedasticity(lm_train$residuals, lm_train$fitted.values) #shows homoscedasticity still
```
Our linear model explains ~89% of the variance in our dataset, and shows an RSE of only 0.73. This indicates that our predictions deviate by only by 0.73 units on an average from the true regression line. 
...
ggplot(db_test_lm, aes(x = db_test_lm$degreeOfMalignancy , y = pred_dom)) +
  ...
  ggtitle("Linear regression model predictions for degree of malignancy") +
  xlab("Actual") +
  ylab("Predicted")+
...
# Conclusion
Hence, using the machine learning analysis, we were able to achieve a 95% accurate random forest and a well performing linear model explaining 89% variance in our dataset which enabled us to efficiently predict both the class and degree of malignancy of a tumor respectively.
